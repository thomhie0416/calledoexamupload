<?php 
    if(isset($_POST['upload'])){
        $maxsize=1033414;
        $format=array('image/jpeg','image/jpg','image/png','image/gif','image/pdf');
    ...
    elseif(!in_array($_FILES['file_upload']['type'],$format)){
        $error_3='Format Not Supported.Only .jpeg,png,gif,pdf,jpg files are accepted';
        echo '<script>alert("'.$error_3.'")</script>';
            ...
            echo "Your file ". basename($_FILES["file_upload"]["name"]). " has been uploaded.";
            }
            else{
                echo "sorry";
                }
